...
template <typename T>
concept LogType = LoggerType<T> || LogUtilType<T>;

template <typename LhsT, typename RhsT>
struct LogUtil {
  using LhsTy = LhsT;
  using RhsTy = RhsT;
  LogUtil(LhsTy &&lhs, RhsTy &&rhs) : lhs(std::forward<LhsTy>(lhs)), rhs(std::forward<RhsTy>(rhs)) {
    static_assert(LogType<LhsTy> && "lhs must be logger or logutil");
  }
  template <typename T>
  auto operator<<(T &&val) {
    return LogUtil<LogUtil<LhsTy, RhsTy>, T>(std::move(*this), std::forward<T>(val));
  }
...
template <typename T>
constexpr size_t getLogUtilSize() {
  size_t ret{0};
...
constexpr auto get(const T &message) {
...
template <LogType T>
constexpr const auto &getLogger(const T &message) {
...
template <int N>
consteval auto generateFormatStr() {
...
// Helper function to create an index sequence and call the function with tuple elements
...
template <LogUtilType M>
decltype(auto) print(M&& t) {
...
struct LoggerBase {
  template <typename T>
  auto operator<<(T &&val) && {
    return ::logger::impl::LogUtil<D, T>(static_cast<D&&>(std::move(*this)), std::forward<T>(val));
  }

  LoggerBase(Level level, const char *fname, int line) : line(line), fname(fname), level(level) {}
...
#define __LOG_INFO logger::impl::LogFinalize{} & ::logger::Logger(::logger::Level::INFO, __FILE__, __LINE__)
#define __LOG_WARNING logger::impl::LogFinalize{} & ::logger::Logger(::logger::Level::WARNING, __FILE__, __LINE__)
#define __LOG_ERROR logger::impl::LogFinalize{} & ::logger::Logger(::logger::Level::ERROR, __FILE__, __LINE__)
#define __LOG_FATAL logger::impl::LogFinalize{} & ::logger::LoggerFatal(::logger::Level::FATAL, __FILE__, __LINE__)
